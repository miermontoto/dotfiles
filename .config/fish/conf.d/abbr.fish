...
abbr -a val 'docker exec -it okticket-api bash -c "composer du -o --quiet; php artisan cache:clear; php artisan optimize; vendor/bin/phpunit --stop-on-failure --stop-on-error --filter BulkImport > /dev/null 2>&1; vendor/bin/phpunit --stop-on-failure --stop-on-error"'
